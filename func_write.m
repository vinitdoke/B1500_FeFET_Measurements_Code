function data_out = func_write(sample_points, sample_interval,i_range)
%     unloadlibrary wgfmu
    ...
    %% Channel 1 pattern
    M = csvread('voltage_pattern_write.csv');
    for i = 1:length(M)
    ...
    calllib('wgfmu', 'WGFMU_addSequence', 101, 'ptn_2', 1);
%% channel 3
    ...
    calllib('wgfmu', 'WGFMU_addSequence', 201, 'ptn_3', 1);
...
%     calllib('wgfmu', 'WGFMU_setMeasureEnabled',102, 7001);
    ...
    calllib('wgfmu', 'WGFMU_setMeasureMode',101, 4001);  % 4000 for Voltage measurement mode
    calllib('wgfmu', 'WGFMU_setMeasureMode',102, 4000); % 4001 for current measurement mode
    calllib('wgfmu', 'WGFMU_setMeasureMode',201, 4001)
    calllib('wgfmu', 'WGFMU_setMeasureMode',202, 4001)
    
    
    calllib('wgfmu', 'WGFMU_setMeasureVoltageRange',102,i_range);  % 5001 for + / -5V
    calllib('wgfmu', 'WGFMU_setMeasureCurrentRange',101,5002); % 6005 for 10mA (default)
    calllib('wgfmu', 'WGFMU_setMeasureCurrentRange',201,i_range);
    calllib('wgfmu', 'WGFMU_setMeasureCurrentRange',202,i_range);
