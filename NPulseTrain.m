...
ns = 1e-9;
    ...
    % PULSE TRAIN (Number of Pulses is a variable, can be changed)
    V_PULSE             = 1.9; % Volts
    t_ON_PULSE          = 0.2*us;
    t_OFF_PULSE         = 0.5*us;
...
% Data Management
main_folder = fileManager('PulseTrain', ([N_MIN N_MAX STEPS V_PULSE t_ON_PULSE]));
...
for i = 1:length(EXPERIMENT_QUEUE)
    ...
    csvwrite([main_folder 'DATA/' 'voltage_pattern_write_' num2str(N) '_pulses.csv'], bw); 
    ...
    % plot with data
    PlotData(PACK_DATA, N, color(i, :), color(i, :));

    % concat data
    full_data_write = [full_data_write; data_out_write];
    full_data_read  = [full_data_read; data_out_read];

    % Save Data for each N
    WRITE_DATA = [T V_gate];
    READ_DATA = [Tr Vr_gate Ir_drain Ir_source Ir_sub];

    csvwrite([main_folder 'DATA/' 'WRITE_' num2str(EXPERIMENT_QUEUE(i)) '_PULSES.csv'], WRITE_DATA);
    csvwrite([main_folder 'DATA/' 'READOUT_' num2str(EXPERIMENT_QUEUE(i)) '_PULSES.csv'], READ_DATA);
...
% save data
csvwrite([main_folder 'DATA/' 'ThresholdAndPulses.csv'], ThresholdAndPulses);
csvwrite([main_folder 'DATA/' 'full_data_write.csv'], full_data_write);
csvwrite([main_folder 'DATA/' 'full_data_read.csv'], full_data_read);

% save figures
saveas(figure(1), [main_folder 'PLOTS/' 'GateVoltage_v_Time.png']);
saveas(figure(2), [main_folder 'PLOTS/' 'DrainCurrent_v_Time.png']);
saveas(figure(3), [main_folder 'PLOTS/' 'SourceCurrent_v_ThresholdVoltage.png']);
saveas(figure(4), [main_folder 'PLOTS/' 'ThresholdVoltage_v_PulseNumber.png']);

% save matlab figures
hgsave(figure(1), [main_folder 'EXTRA/' 'GateVoltage_v_Time.fig']);
hgsave(figure(2), [main_folder 'EXTRA/' 'DrainCurrent_v_Time.fig']);
hgsave(figure(3), [main_folder 'EXTRA/' 'SourceCurrent_v_ThresholdVoltage.fig']);
hgsave(figure(4), [main_folder 'EXTRA/' 'ThresholdVoltage_v_PulseNumber.fig']);
